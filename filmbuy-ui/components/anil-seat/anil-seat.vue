	...
	<view class="sel-seat" :style="{height}">
		...
		<view class="seat-main">
			<movable-area class="vm-area">
				<movable-view :style="'width: 750rpx;height:'+(seatRow*40+350)+'rpx;'" :inertia="true" :scale="true"
					...
					<!-- seat content -->
					<view v-for="(item,index) in seatArray" :key="index" class="sm-cell"
						...
						<view v-for="(seat,col) in item" :key="col" class="dp-ib"
							...
							<template v-if="seat.flag === 1">
								<image v-if="seat.type===0" class="sm-icon"
									src="/uni_modules/anil-seat/static/unselected-l.png" mode="aspectFit"></image>
								<image v-else-if="seat.type===1" class="sm-icon"
									src="/uni_modules/anil-seat/static/selected-l.png" mode="aspectFit"></image>
								<image v-else-if="seat.type===2" class="sm-icon"
									src="/uni_modules/anil-seat/static/bought.png" mode="aspectFit"></image>
							</template>
							<!-- // 情侣右座 -->
							<template v-else-if="seat.flag === 2">
								<image v-if="seat.type===0" class="sm-icon"
									src="/uni_modules/anil-seat/static/unselected-r.png" mode="aspectFit"></image>
								<image v-else-if="seat.type===1" class="sm-icon"
									src="/uni_modules/anil-seat/static/selected-r.png" mode="aspectFit"></image>
								<image v-else-if="seat.type===2" class="sm-icon"
									src="/uni_modules/anil-seat/static/bought.png" mode="aspectFit"></image>
							</template>
							<!-- // 普通座位 其他座位 -->
							<template v-else>
								<image v-if="seat.type===0" class="sm-icon"
									src="/uni_modules/anil-seat/static/unselected.png" mode="aspectFit"></image>
								<image v-else-if="seat.type===1" class="sm-icon"
									src="/uni_modules/anil-seat/static/selected.png" mode="aspectFit"></image>
								<image v-else-if="seat.type===2" class="sm-icon"
									src="/uni_modules/anil-seat/static/bought.png" mode="aspectFit"></image>
							</template>
						</view>
			...
			<view class="fix-tips">
				<view class="v-tips">
					<image :style="'width:'+seatSize+'px;height:'+seatSize+'px'"
						src="/uni_modules/anil-seat/static/unselected.png" mode="aspectFit"></image>
		...
		<view class="seat-foot">
			...
			<view class="f-btn" :class="{disabled: SelectNum === 0 }" @tap="buySeat">
