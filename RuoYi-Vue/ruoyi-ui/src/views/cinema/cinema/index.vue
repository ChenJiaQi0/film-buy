    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['cinema:cinema:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['cinema:cinema:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listCinema, getCinema, delCinema, addCinema, updateCinema } from "@/api/cinema/cinema";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCinema(this.queryParams).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除cinema编号为"' + ids + '"的数据项？').then(function() {
        return delCinema(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('cinema/cinema/export', {
        ...this.queryParams
      }, `cinema_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
